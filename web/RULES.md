...
1. Two teams, Team A and Team B. The team with the most points at the end wins.
2. Somebody is lying. In a normal game, one player on each team is secretly a **Mole** who wants the *other* team to win. In a small game (4-6 players) there is only one Mole, somewhere.
...
1. The host **does not play** and is not on either team.
2. The host hides the **3 special discs**. Only the host knows where they are.
3. The host may give at most **3 minor clues** and **2 big clues** for the **whole game**. The host gives clues **out loud**. The app never gives clues.
4. A **minor clue** narrows it down: a floor, a kind of room, warmer or colder. A **big clue** names the room, or the hiding spot itself.
5. The host decides when to give a clue and which team hears it.
6. The host **may choose which 3 powers are live** this game, instead of keeping the computer's pick.
7. The host **may pick the Moles by hand** instead of keeping the random pick. Both choices have to happen on the host's turn with the laptop, before roles are read.
8. The host keeps everything secret: where the special discs are, which powers are live, and who the Moles are.
9. With no host, the computer picks the 3 powers, the players hide all the discs, and nobody knows which ones are special.

## Setup

1. **Register the discs.** Put each disc in the tray once. The game remembers them forever, so you only do this once.
...
1. **3 discs are special each game.** There are 7 powers in all. The computer picks which 3 are live, or the host does.
2. The screen tells you when you've claimed a special disc, and its task is harder and longer.
3. **End-of-game powers** (CONFESSION, VETO, TESTIMONY, SHIELD, LONG COUNT): if the room accepts the task at the reel, your team **banks the power**. These can only be used **after the reel**.
4. **Play powers** (HIJACK, SCRATCH): your team holds one the moment you log its task, and it is only ever used **during play**.
...
| SCRATCH | During play, the other team's next disc loads slowly before its task appears. |
...
1. You can use a SCRATCH at any time during play, even while your own team has a task running.
2. Use it from your phone, or type your team code under *use scratch* on the laptop.
3. It hits the **next** disc the other team claims. Their phones and the laptop warn them it's coming.
4. When they claim that disc, it **loads for 45 seconds**. Nobody sees the task until it finishes loading, and a loading task can't be logged or hijacked.
5. The loading time comes **off their clock**. The tray is not blocked while it loads.
6. A team can only have one scratch waiting on it at a time.
...
1. After powers are spent, each team names the Mole **on their own team**. A team that used CONFESSION already knows.
2. Named correctly: **+25** to that team.
3. Named wrong, or never named: **+25 to the other team**.

## Winning

The team with the most points after the accusations wins. A tie is a draw.
